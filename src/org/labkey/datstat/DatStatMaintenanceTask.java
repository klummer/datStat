 ...
 */
public class DatStatMaintenanceTask implements SystemMaintenance.MaintenanceTask
{
    private Logger _log = Logger.getLogger(DatStatMaintenanceTask.class);
    ...
    public void run()
    {
        ...
        _log.info("DATStat Import Task starting cycle...");
        ...
        for (String containerId : _datStatContainerIds)
        {
                ...
                if (Encryption.isMasterEncryptionPassPhraseSpecified())
                {
                    ...
                    if (map.containsKey(DatStatManager.DatStatSettings.Options.enableReload.name()) && Boolean.parseBoolean(map.get(DatStatManager.DatStatSettings.Options.enableReload.name())))
                    {
                        ...
                        if ((today.getTime() - reloadDate.getTime()) % dayInterval == 0)
                        {
                            ...
                            {
                                _log.error("An error occurred exporting from DATStat", e);
                            }
                        }
                    }
                }
            ...
            catch (ImportException e)
            {
                ...
                _log.error("DATStat import failed" + message);
            }
            catch (Throwable t)
            {
        ...
        _log.info("DATStat Import Task finished cycle in " + String.valueOf((System.currentTimeMillis() - msStart) / 1000) + " seconds.");
    }
